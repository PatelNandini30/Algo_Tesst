...
async def algotest_backtest(
    request: BacktestRequest,
    x_no_cache: Optional[str] = Header(None, alias="X-No-Cache"),
    ...
    """
    if x_no_cache and x_no_cache.lower() == "true":
...
async def dynamic_backtest(
    ...
    """
    # Check for no-cache flag in request, X-No-Cache header, or Cache-Control: no-cache
    header_no_cache = (x_no_cache and x_no_cache.lower() == "true") or \
                      (cache_control and "no-cache" in cache_control.lower())
    no_cache = request.get('no_cache', False) or header_no_cache
    ...
    try:
        ...
        class DynamicStrategyRequestObj:
            def __init__(self, data):
                ...
                # Super Trend filter
                self.super_trend_config = data.get("super_trend_config", "None")
        ...
        # Map super_trend_config to SuperTrendConfig enum
        super_trend_mapping = {
            "None": "None",
            "5x1": "5x1",
            "5x2": "5x2"
        }
        super_trend_value = super_trend_mapping.get(getattr(request_obj, 'super_trend_config', 'None'), 'None')
        
        strategy_def = StrategyDefinition(
            name=request_obj.name,
            legs=legs,
            parameters=request_obj.parameters,
            super_trend_config=super_trend_value
        )
