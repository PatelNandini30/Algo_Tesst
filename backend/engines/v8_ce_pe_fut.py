...
def run_v8_ce_pe_fut(params: Dict[str, Any]) -> Tuple[pd.DataFrame, Dict[str, Any], Dict[str, Any]]:
    """
    Run V8 strategy: CE Sell + PE Buy + Future Buy
    All 4 expiry windows supported
    """
    # Load required data
    ...
    # Filter spot to base2 ranges (inside ranges for V8 CE PE FUT)
    ...
    for _, exp_row in exp_df.iterrows():
        ...
        for from_date, to_date in intervals:
            ...
            put_strike = round_half_up((call_strike * (1 - put_strike_pct_below/100)) / 50) * 50
...
def run_v8_main1(params: Dict[str, Any]) -> Tuple[pd.DataFrame, Dict[str, Any], Dict[str, Any]]:
    """V8 CE PE FUT with weekly expiry window"""
    params['expiry_window'] = 'weekly_expiry'
    return run_v8_ce_pe_fut(params)


def run_v8_main2(params: Dict[str, Any]) -> Tuple[pd.DataFrame, Dict[str, Any], Dict[str, Any]]:
    """V8 CE PE FUT with weekly_t1 expiry window"""
    params['expiry_window'] = 'weekly_t1'
    return run_v8_ce_pe_fut(params)


def run_v8_main3(params: Dict[str, Any]) -> Tuple[pd.DataFrame, Dict[str, Any], Dict[str, Any]]:
    """V8 CE PE FUT with weekly_t2 expiry window"""
    params['expiry_window'] = 'weekly_t2'
    return run_v8_ce_pe_fut(params)


def run_v8_main4(params: Dict[str, Any]) -> Tuple[pd.DataFrame, Dict[str, Any], Dict[str, Any]]:
    """V8 CE PE FUT with monthly expiry window"""
