...
def run_v1(params: Dict[str, Any]) -> Tuple[pd.DataFrame, Dict[str, Any], Dict[str, Any]]:
    """
    Run V1 strategy: CE Sell + Future Buy
    ...
    # Filter spot to base2 ranges (inside ranges for V1)
    ...
    for _, exp_row in exp_df.iterrows():
        ...
        for from_date, to_date in intervals:
            ...
            call_strike = round_half_up((entry_spot * (1 + params.get("call_sell_position", 0.0)/100)) / 50) * 50
...
def run_v1_main1(params: Dict[str, Any]) -> Tuple[pd.DataFrame, Dict[str, Any], Dict[str, Any]]:
    """V1 with weekly expiry window"""
    params['expiry_window'] = 'weekly_expiry'
    return run_v1(params)


def run_v1_main2(params: Dict[str, Any]) -> Tuple[pd.DataFrame, Dict[str, Any], Dict[str, Any]]:
    """V1 with weekly_t1 expiry window"""
    params['expiry_window'] = 'weekly_t1'
    return run_v1(params)


def run_v1_main3(params: Dict[str, Any]) -> Tuple[pd.DataFrame, Dict[str, Any], Dict[str, Any]]:
    """V1 with weekly_t2 expiry window"""
    params['expiry_window'] = 'weekly_t2'
    return run_v1(params)


def run_v1_main4(params: Dict[str, Any]) -> Tuple[pd.DataFrame, Dict[str, Any], Dict[str, Any]]:
    """V1 with monthly expiry window"""
    params['expiry_window'] = 'monthly_expiry'
    return run_v1(params)


def run_v1_main5(params: Dict[str, Any]) -> Tuple[pd.DataFrame, Dict[str, Any], Dict[str, Any]]:
    """V1 with monthly_t1 expiry window"""
