...
- **Hardware target** (encoded in `docker-compose.yml`): 16 GB RAM, HDD (not SSD). Memory limits and Postgres tuning are deliberately conservative — do not raise `shared_buffers`, `work_mem`, or worker counts without revisiting the budget comment at the top of `docker-compose.yml`.
...
- The Postgres tuning is HDD-aware: `random_page_cost=4.0`, `effective_io_concurrency=2`, `max_parallel_workers=2`. More parallel workers makes things **slower** on this hardware.
...
- **Never** raise Docker memory limits without updating the budget header in `docker-compose.yml`. The total must fit in 16 GB with HDD swap headroom.
